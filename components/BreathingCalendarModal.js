...
import {
  getBreathingCalendarData,
  getBreathingSessionsByDate,
  getMonthlyBreathingStats,
  getWeeklyBreathingData,
} from "../utils/breathingManager";

const { width } = Dimensions.get("window");

const BreathingCalendarModal = ({ visible, onClose }) => {
  ...
  const loadCalendarData = async () => {
    ...
    const data = await getBreathingCalendarData(year, month);
    const stats = await getMonthlyBreathingStats(year, month);
  ...
  const loadWeeklyData = async () => {
    ...
    const data = await getWeeklyBreathingData(weekStart);
    setWeeklyData(data);
  };
  ...
  const handleDatePress = async (day) => {
    ...
    const sessions = await getBreathingSessionsByDate(dateString);
          ...
          >
            ...
            {day.sessionCount > 0 && (
              <View style={styles.sessionBadge}>
                <Text style={styles.sessionBadgeText}>{day.sessionCount}</Text>
              </View>
            )}
          </TouchableOpacity>
        ))}
  ...
  const renderHeatMap = () => {
    const maxCount = Math.max(...weeklyData.map((day) => day.sessionCount), 1);
      ...
      <View style={styles.heatMapContainer}>
        ...
        <View style={styles.heatMapGrid}>
          {weeklyData.map((day, index) => {
            const intensity = day.sessionCount / maxCount;
            ...
            return (
              <View key={index} style={styles.heatMapDay}>
                ...
                <Text style={styles.heatMapBreaths}>{day.totalBreaths}🫁</Text>
              </View>
            );
          })}
        </View>
    ...
    return (
      <View style={styles.selectedSessionsContainer}>
        ...
        <Text style={styles.selectedDateSubtitle}>
          {selectedSessions.length} breathing session
          ...
          {selectedSessions.map((session, index) => (
            <View key={session.id} style={styles.sessionItem}>
              ...
              <View style={styles.sessionDetails}>
                <Text style={styles.sessionBreaths}>
                  🫁 {session.breathCount} breaths
                </Text>
                <Text style={styles.sessionType}>5-breath session</Text>
              </View>
            </View>
          ))}
        </ScrollView>
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>🧘 Breathing Calendar</Text>
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: "#93D5E1", // Blue theme for breathing
  },
  header: {
    flexDirection: "row",
    alignItems: "center",
...
export default BreathingCalendarModal;
